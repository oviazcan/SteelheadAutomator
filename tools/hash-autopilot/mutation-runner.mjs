...
export async function runMutationCycle(page, route, sentinelsConfig, sink, deps) {
  ...
  // como centinela — el fail-closed del mutate sería inútil si el finally muta igual.
  let verified = false;
  try {
    ...
    // Restaurar SOLO si verificamos la identidad (nunca tocar un objeto no-centinela);
