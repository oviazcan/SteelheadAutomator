...
| `load-calculator` (Calculadora de Piezas por Carga) | 0.2.0 (**Fase 1 validada en vivo**: Configurador de Estaciones — datos maestros de tina/línea en `customInputs` de estación vía `CreateStationInputSchema`+`UpdateStationInputs`, por estación o bulk por línea; popup→`openStationConfig`. Motor de cálculo puro cuadrícula/área/barril validado vs golden del `Calculo.xlsx` (87/112/112·47/123/105) + núcleo RMW no-destructivo, 30 tests. **Fase 2a/2b implementadas y deployadas**: calculadora en el modal de Rack Types (`CreateEditPartsPerRackTypeQuery`) + persistencia parcial en el PN (`UpdatePartNumber`); **F2c** (preview de geometría) implementada con escritura deshabilitada (`F2C_WRITE_ENABLED=false`). **Pendiente validación en vivo de F2a/F2b/F2c.**) | [`docs/applets/load-calculator.md`](docs/applets/load-calculator.md) |
...
| `bulk-upload` | 1.5.39 + config 1.7.119 + ext 1.6.5 (**2026-07-15**: troceo #4 SOLO_PN **VALIDADO en vivo** — corrida real de 20 831 NP en 21 lotes; fix `pnLookup` TDZ ("Cannot access 'pnLookup' before initialization" con troceo SOLO_PN activo); matcher de parámetros de Espesor tolerante (trim del catálogo + fallback numérico `numSig`). Arrastra el **Refactor F1-F5 + `sa_load_history`→IndexedDB** — ver bitácora §"Cierre de sesión 2026-06-07": **F1** módulos puros + golden tests (`bulk-upload-parse.js`/`bulk-upload-classify.js`); **F2** memory hardening compartido (`host-cleanup-shared.js`) + `AbortController` en `steelhead-api`; **F4** batches AddParams/SaveQuoteLines (N→1) + fix `isDefaultPartNumberPrice`; **F5** `classifyRunIntent` + badge de intención (SOLO_PRECIO/AJUSTE_LINEA/ENRIQUECIMIENTO/ALTA). `sa_load_history` migrado a IndexedDB — **VALIDADO en vivo** (7 corridas, localStorage migrado). **Gotcha de deploy de extensión:** bumpear `extension/manifest.json` Y `config.extensionVersion` JUNTOS y verificar el manifest DENTRO del zip servido (no solo bytes); Chrome lee del manifest, no del config. Ver `docs/applets/bulk-upload.md` para el detalle completo (histórico de precios, ControlCambios, chunking de cotizaciones, etc.). | [`docs/applets/bulk-upload.md`](docs/applets/bulk-upload.md) |
...
| `wo-mover` (mover OTs entre OVs) | 0.2.1 (rename de schema SH: `CreateUpdateWorkOrdersChecked` usa `externalNotes` en vez de `customerFacingNotes`; wo-mover lee `externalNotes` con fallback. Por lo demás, solo reasigna encabezado; la parte/PT queda en la OV origen y se asocia manual — la UI no expone la asociación por API) | [`docs/applets/wo-mover.md`](docs/applets/wo-mover.md) |
| `wo-completer` (Completar / Descompletar OTs) | 0.1.0 (panel dark-mode desde popup; pega listado de OTs `idInDomain` + toggle. **Completar** = `AddPartsToWorkOrders` tipo `COMPLETE` por cada cuenta activa de `WorkOrder{idInDomain}`; **Descompletar** = `CreateManyPartsTransfersChecked` tipo `REVERT_COMPLETE` por cada COMPLETE vivo. Motor puro + golden 10/10. **Deployado a gh-pages (config 1.7.38); pendiente run real**) | [`docs/applets/wo-completer.md`](docs/applets/wo-completer.md) |
| `auto-router` (Auto-Ruteador) | 0.2.0 (**Fase 1 VALIDADA en vivo**: re-rutea una WO entre líneas, ej. T204→T205, con regla **bypass→role-match→reúso de proceso→momentum serpentino** (22/22 rutas críticas exactas, golden test). **Fase 2** (batch multi-orden manual) + **Fase 2b** (captura multi-orden desde el board) + **Fase 3** (ruteo directo desde el Scheduling board sin modal, incl. "Rutear TODAS") **implementadas y deployadas** (config hasta 1.7.6+); tooltip enriquecido de Metal base+PS en el board (`board-metal-tooltip.js`). Pendiente: Fase 0 opcional (fidelidad de test vs `SearchStationsForTreatment`)) | [`docs/applets/auto-router.md`](docs/applets/auto-router.md) |
| `surtido-guard` (Candado de Surtido Programado) | 0.1.2 (config **1.7.158**, DEPLOYADO 2026-07-20. **toggle + bloqueo + MARCADO VERDE validados en vivo**. Verde 2026-07-20: idioma-indep (ancla la tarjeta por `data-steelhead-component-id="WORKBOARD_PAGE_WORKBOARD_CARD_SALES_ORDER_LINK"`), **edge-to-edge** (tinte sólido `#e2f4ec` sobre el cuerpo blanco full-width) y **rápido** (`kickDecorate` + `scheduleDecorate` por `requestAnimationFrame` + observer acotado: el candado del modal solo corre con diálogo abierto, no `querySelectorAll` de doc cada 80ms en scroll; scan del verde sobre `document` para cubrir TODAS las secciones de step). v0.1.1: estado `enforcementEnabled` movido a `window.__saSurtidoGuardEnabled` singleton — antes vivía en el closure y `injectAppScripts` re-evaluaba el IIFE en cada acción del popup creando una instancia nueva, así que apagar el candado mutaba una instancia distinta a la que tenía el interceptor de fetch latcheado → "Desactivado" sin efecto. Test `surtido-guard-toggle.test.js`. Bloquea mover piezas NO programadas del step "Preparando Surtido en Almacén" al siguiente proceso. **Programada = la pieza tiene tarea en el programa** — `GetRelatedScheduleData` → `scheduleTaskElement.associatedPartsTransferAccounts.id` (= `fromAccountId` de la mutación). Interceptor de `fetch` bloquea `CreateManyPartsTransfersChecked` tipo STEP (cubre **modal MOVER y drag silencioso**) devolviendo error GraphQL sintético + toast; agrisa botones del modal; marca verde las tarjetas con "Tareas Programadas:". Scope por nombre de nodo (`allRecipeNodes`). **Fail-safe** ante dato faltante. Toggle no persistente en popup, default ON. Core puro 13/13 golden. Pendiente: validación fina del bloqueo (falsos positivos prog/no-prog) + drag silencioso) | [`docs/applets/surtido-guard.md`](docs/applets/surtido-guard.md) |
...
| `vale-almacen` (Vale de Almacén) | 0.1.2 (FAB 📦 en Producción/Mantenimiento/Sensores/Inventario → panel multi-línea que emite un evento de mantenimiento sobre nodo **Surtimiento** (raíz + 3 pasos; paso 0 = sensores `NUMBER`/TEXT/BOOLEAN = artículos con cantidad vía `measurement`, incl. EPP). Cada línea `artículo+cantidad+usuario` queda como **comentario estructurado parseable** `[VALE]…[/VALE]` (motor puro `vale-almacen-engine.js`, 19 tests) con núm. de empleado (`UserDialogQuery`→`customInputs.DatosLaborales.CodigoEmpleado`); "Asignado"=quien recoge. **v0.1.1/0.1.2**: el vale = **solo la solicitud** — llena y completa el paso 0 (sin archivarlo) y **deja el evento VIVO** (solo se cierra al DESCARTAR); antes archivaba el paso recién ejecutado (deshaciéndolo) y completaba el evento de más. **Deployado a gh-pages** (hashes+app desde config 1.7.20; fixes 0.1.1/0.1.2 deployados hasta config 1.7.43) + incluido en bundle Safari/iPad desde v0.3.0 (FAB). Pendiente run real) | [`docs/applets/vale-almacen.md`](docs/applets/vale-almacen.md) |
...
1. **Integridad de scripts remotos (ALTO) — CÓDIGO EN MAIN (mergeado 2026-07-09, `570131e`; fail-open hasta Fase 2):** `extension/background.js` ejecutaba `new Function(c)()` sobre código de GitHub Pages sin verificar. **Solución elegida: firma criptográfica ECDSA P-256** (no solo hashes-en-config, que no detiene un compromiso de repo/Pages). `config.json` firmado (`config.sig` separado) + `scriptIntegrity` por script; verificación fail-closed en `background.js`; privada en **GCP KMS del cliente** (handoff = cambio de IAM), pública embebida. Ver bitácora [`docs/applets/security-integrity-signing.md`](docs/applets/security-integrity-signing.md). **Pendiente:** Fase 0 (provisionar KMS) + Fase 2 (republicar extensión), ambas manuales del usuario.
...
6. **Anclajes bilingües ES+EN (audit) — RE-AUDITADO 2026-07-17; ~7 clusters cerrados, 9 anclas de deuda real bloqueadas por evidencia.** Mapa verificado contra archivos (el mapa 2026-07-15 quedó desfasado) en [`docs/architecture/bilingual-anchoring-debt.md`](docs/architecture/bilingual-anchoring-debt.md). **Cerrados** (commits 07-16/07-17): price-confirm-guard (`root_DatosPrecio`), invoice-autofill (gate `root_DatosContables`+heading bilingüe), invoice-auto-regen (×4 bilingües), cfdi-attacher (`data-testid`+`MuiSwitch`), create-order-autofill (ship-to), unit-autoconvert (Panel B por posición + modo). **Corrección importante:** `surtido-guard` NO tiene falla de seguridad en inglés — el **bloqueo es API-driven/idioma-indep** (`modalShouldBlock` + `findMoveDialog` bilingüe); solo el **marcado verde cosmético** es mono-idioma ES (P3). **Deuda real restante: 8 anclas** (P2: bill-autofill gate, invoice-autofill income col, unit-autoconvert Panel A; P3: surtido-guard verde cosmético, price-confirm-core, invoice-autofill "Line #", invoice-default-tab, load-calculator-modal). `proceso-calculator` NO es deuda (ya ancla por `data-steelhead-component-id` primario). Cada una **bloqueada por evidencia** (string del otro locale o wrapper HTML del modal); la vía preferida en modales de NP es reanclar por `data-steelhead-component-id` (patrón ya vivo en proceso-calculator). Regla dura: no adivinar.
   **ACTUALIZACIÓN 2026-07-20 (DEPLOYADO a producción):** con la evidencia HTML del usuario se cerró el grueso. **Estándar de anclaje** fijado en [`docs/architecture/dom-patterns.md`](docs/architecture/dom-patterns.md) (jerarquía: estructura idioma-indep > texto bilingüe > nunca CSS hasheado). **Inventario completo** (workflow 50 agentes) en [`docs/architecture/anchor-migration-inventory.md`](docs/architecture/anchor-migration-inventory.md): 73 anclas de SH-texto, **61 ya cumplían**, resto migrado/resuelto. **Migrados a estructura + DEPLOYADOS** (config **1.7.150→1.7.158**, firmados KMS, byte-a-byte verificado): bill-autofill (gate `root_DatosContables_Divisa`), cfdi-attacher (3er path structMatch), invoice-default-tab (`button[value="packing-slips"]`), proceso-calculator ficha (`PART_NUMBER_PAGE_PROCESS_SETUP`), load-calculator-modal (por nº de parte del título — **bug del modal Edit resuelto**), unit-autoconvert Panel A (`Core.isConvertible` + estructura), surtido-guard verde (component-id + edge-to-edge). **Hallazgo clave:** `data-steelhead-component-id` es atributo real/vivo de SH y los modales CONTABLES muestran labels en INGLÉS aun con instancia ES (income/"Line #"/"/Part:" ≠ deuda real). **Deploy desde workbench:** workbench estaba desfasado (config 1.7.118 vs main 1.7.149) → NO merge en bloque; se reconcilió cada delta sobre main + `deploy.sh`/`wb-deploy.sh` (protege WIP ajena). Docs de anclas viven en `workbench`; **falta llevarlas a `main`**.
...
**Nota (2026-07-15):** items 2-7 hechos en la rama `workbench`, verificados con suite 62/0. Falta llevarlos a `main` + deploy. También se corrigió un test rojo pre-existente del `auto-router` (`destinationLines` seguía excluyendo la línea origen — bug Image #6 vivo; ver bitácora del applet).
