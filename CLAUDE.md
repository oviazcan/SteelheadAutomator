...
|---|---|---|
| `bulk-upload` | 1.5.20 + config 1.6.38 (**actualización de precios** — piloto OK. **Feature A:** upload con acabados vacíos → MODIFICA el PN activo más reciente (id más alto); auto si 1 candidato, confirmar si 2+; `confidence:'name+blank-csv-recent'` va ANTES de `labelsMatchFull`; no dispara si trae acabados no vacíos. **Feature B:** footprint en `customInputs.ControlCambios` (campo del schema 3932, `{Fecha,Usuario,Accion,Detalle,Version}`), 1 entrada/corrida solo si hubo cambio real; `Accion`=ALTA/PRECIO/ENRIQUECIMIENTO; `Usuario`=`CurrentUserDetails`; `Version`=`window.REMOTE_CONFIG.version` (NO `bulkCfg()`). **Fix clave:** `inputSchemaId` dinámico (`runtimeInputSchemaId=latestSchema.id`) en los 5 SavePartNumber — deja de degradar PNs a 3456, migra a 3932 al tocarlos. Helpers puros en `bulk-upload-cc.js` (+19 tests `node --test`). Gotcha: colisión de versión con deploy paralelo del archiver (1.6.37) → subió a 1.6.38. Pendiente: `precioAnterior` real para delta en `Detalle`. Previo 1.5.19 — línea de cotización heredaba sin proceso (FK-fallback a `existingProcessId`); 1.5.18 — "Preservar proceso" borraba `defaultProcessNodeId`) (+ ext 1.6.2) | [`docs/applets/bulk-upload.md`](docs/applets/bulk-upload.md) |
