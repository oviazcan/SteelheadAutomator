...
| `pn-specs-column` (Specs en Números de Parte) | 0.1.0 **DEPLOYADO** config 1.7.85 (autoInject en `/PartNumbers`: **toggle persistente en el header** + **columna "Specs / Params num."** que enriquece cada NP visible con sus specs y **parámetros NUMÉRICOS** — nombre+rango+unidad, ej. `Espesor 1.27–3.5 µm`; excluye BOOLEAN/DROPDOWN y archivados. **`AllPartNumbers` NO trae specs/params → 2º query `GetPartNumber` por NP** (opt-in + memory-hardening: cache slim, mem monitor+guardrail, pool 4×/~7req/s, Datadog stop, Apollo drain). Core 14/14 golden + validado en vivo contra producción (`44068-205-01`→`E27550 (Plata): Espesor 1.27–3.5 µm`); scripts servidos byte-exact. **Hallazgo:** el hash `GetPartNumber` rotó (`8e3fdb52…`→`5efd689d…`, HTTP 400→200); actualizado en config y deployado. **Pendiente run real integrado** (validación en foreground por el usuario; la tab automatizada en background congela los fetches)) | [`docs/applets/pn-specs-column.md`](docs/applets/pn-specs-column.md) |
