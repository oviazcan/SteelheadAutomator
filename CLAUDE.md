...
| `wo-completer` (Completar / Descompletar OTs) | 0.1.0 (panel dark-mode desde popup; pega listado de OTs `idInDomain` + toggle. **Completar** = `AddPartsToWorkOrders` con transfer `type:"COMPLETE"` por cada cuenta activa de `WorkOrder{idInDomain}`, sin validar nodo. **Descompletar** = `CreateManyPartsTransfersChecked` `type:"REVERT_COMPLETE"` por cada COMPLETE vivo de `GetWorkOrderPartsTransfers`. Motor puro + golden byte-a-byte del scan (10 tests); `at` del revert normalizado a millis+Z. Hashes nuevos: `WorkOrder`/`GetWorkOrderPartsTransfers`/`CreateManyPartsTransfersChecked`. Dry-run→preview→ejecutar. **Deployado config 1.7.38; pendiente run real**) | [`docs/applets/wo-completer.md`](docs/applets/wo-completer.md) |
