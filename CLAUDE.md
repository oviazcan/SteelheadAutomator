...
6. **Anclajes bilingües ES+EN (audit) — ✅ MAPA COMPLETO; hardening bloqueado por evidencia del usuario.** 25 anclajes mono-idioma en 12 applets, priorizados por riesgo en [`docs/architecture/bilingual-anchoring-debt.md`](docs/architecture/bilingual-anchoring-debt.md). P1 seguridad: `surtido-guard` (candado no bloquea con SH en inglés). No se hardeniza sin el string real del otro locale (regla dura: no adivinar).
