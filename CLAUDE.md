...
|---|---|---|
| `bulk-upload` | 1.5.19 + config 1.6.36 (**línea de cotización heredaba sin proceso** — `partNumberPrice.processId` nacía vacío cuando `part.processId` quedaba null/undefined por processCache miss, aunque el PN sí tuviera default; red de seguridad → FK-fallback a `existingProcessId` en los 2 puntos de SaveManyPNP, salvo `-`/clearDefaultProcess. Incidente Tipsa cotiz. 233/235; **no recuperadas** por estar ligadas a OV. Pendiente: causa raíz del cache miss + piloto. Nota: `GetQuote_v71` usa vars `{idInDomain,revisionNumber}`. Previo 1.5.18 — **fix CRÍTICO: "Preservar proceso" borraba `defaultProcessNodeId`**: Call B FK-fallback + distingue heredar vs `-`/borrar; Tipsa/TROQUELADOS 17 PNs recuperados. STEP 6b preserve `optInOuts`/UCs; cascade STEP 6a; gotcha predictivo `0` no borra, usar `-`) (+ ext 1.6.2) | [`docs/applets/bulk-upload.md`](docs/applets/bulk-upload.md) |
