...
| `schedule-batch-highlighter` (Resaltar Lote en Programación) | 0.1.1 (**DEPLOYADO config 1.7.171, tag `v1.7.171`, "a mi riesgo" del operador; pendiente validar en vivo el fix**). Workaround del **bug de Steelhead** (reportado) en el filtro nativo "Received Batches" del **Schedule Board** (`/Schedules/<id>/ScheduleBoard/<id>`): su dropdown solo ofrece UN id por nombre → al filtrar un lote **homónimo** esconde las tareas de los OTROS lotes con ese nombre (validado: "210726" pasó de 2 a 1 tarea). El filtro nativo es **100% client-side (0 queries, no cambia URL)** → no reutilizable, y **ocultar** filas pelea con la **virtualización** de la tabla. Por eso NO filtra: **panel flotante dark-mode** (`position:fixed`, no inyectado en React para no congelar la SPA) donde tecleas el NOMBRE → **RESALTA** (verde + borde) las filas cuyo "Received Batches" coincide (todas las homónimas) y **MARCA su checkbox** (`cb.click()`, sin tocar React internals), con **aviso de scroll** (la tabla virtualiza → solo alcanza las filas en DOM). Limpiar des-marca SOLO lo que puso el applet (`S.checkedByUs`). Detección de columna por **alineación X**: la tabla es **MUI CSS-grid** y el header "Received Batches" es un **`<strong>` hoja dentro de un `<td>`** (no `<th>`) → el glue matchea el nodo hoja, **sube al `<td>` ancestro** para medir la columna del grid (no el `<strong>`, que solo mide el texto) y por fila toma la celda cuyo centro X esté más cerca (tol. 60px); sin header = fail-safe. **Bug del 1er deploy (1.7.170):** el selector no incluía `strong`/`td` → "No encuentro la columna" → corregido en 0.1.1. Core puro `isScheduleBoardUrl`/`rowMatchesBatchName`/`extractBatchNames`/`countMatches`, **12/12 golden**. 100% DOM (no usa `steelhead-api.js`), `autoInject:true`. | [`docs/applets/schedule-batch-highlighter.md`](docs/applets/schedule-batch-highlighter.md) |
