...
| `report-regen` | 0.1.0 (botón ♻️ "Regenerar Reportes" en header secundario, ancla play+correo; cooldown GLOBAL del domain vía `GetRecomputableAt.recomputableAt` server-side, sin backend propio; `GenerateDuckDb`+`JobQuery`; **auto-gating por `MANAGE_REPORTING` en runtime** — autoInject NO respeta requiredPermissions; popup vía handler genérico `fn`, sin tocar `extension/`; **pendiente run real**) | [`docs/applets/report-regen.md`](docs/applets/report-regen.md) |
