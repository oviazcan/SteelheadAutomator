...
**Nota (2026-07-15):** items 2-7 **DEPLOYADOS a producción, config 1.7.120, tag `v1.7.120`** (scripts remotos verificados en vivo). También se corrigió un test rojo pre-existente del `auto-router` (`destinationLines` seguía excluyendo la línea origen — bug Image #6 vivo). **PENDIENTE de propagar a otros canales:** (a) cambios de `extension/` (CSP en manifest, XSS en `popup.js`, gate en `content.js`) requieren **rebuild + republicar el `.zip`** (bump `extensionVersion`, Chrome Web Store); (b) **bundle Safari/iPad** desactualizado — `tools/build-safari.sh` + recompilar Xcode.
