...
' V15.6 (2026-08-07): la plantilla se entrega PROTEGIDA. Esta macro es la UNICA de las cinco
'   que NO escribe en el libro --solo lee Upload y CAT_Productos y vuelca a un libro NUEVO--,
'   asi que en teoria no la afecta la proteccion. Aun asi se envuelve el sec.8 con
'   SA_Desproteger/SA_Proteger: el libro se entrega tambien con la ESTRUCTURA protegida y
'   sec.8 es la unica parte que crea y manipula un libro. Cuesta dos llamadas.
'   Lease al reves: si ExportarCSV falla con la plantilla protegida, la causa NO esta aqui.
'   El sospechoso real es Worksheet_SelectionChange, que truena en CADA clic sobre Proceso
'   o Spec 1-4 si CAT_Procesos/CAT_Specs estan protegidas (lo cubre ThisWorkbook).
' V15.5 (2026-08-05): (a) varios clientes en COTIZACION+NP dejan de BLOQUEAR el export --
'   el applet crea una cotizacion por cliente desde hace versiones y la plantilla lo vetaba;
'   ahora es confirmacion, no veto. (b) la version del aviso final se lee del titulo de la
'   hoja (TplVer) en vez de estar escrita a mano: decia "CSV v12 exportado" en la v13.
'   sec.3 es header-driven, asi que el modulo sirve a v12 y v13 sin cambios de posiciones.
' V15.4 (2026-07-03): FIX carga por Id SH -- CSV salia SIN DATOS cuando las filas se
'   identifican por "Id SH" (col C) con "Numero de parte" (col E) VACIO (caso de uso legitimo:
...
'   Inconsistencia interna: el rango reconocia Id SH pero la escritura lo ignoraba. Ahora el
'   criterio de "fila con datos" es PN <> "" OR Id SH <> "" en los TRES puntos (conteo nOut,
'   loop de escritura, aviso SOLO_PN > 2000), identico al parser downstream
'   (`bulk-upload.js`: `if (!pn && !idShEarly) continue`). Ademas la clave de orden desempata
'   por Id SH para que las filas solo-IdSH salgan deterministas. Ver sec.sec.2, 4, 6 y HasRowData.
...
'   error)" porque escribir un archivo NUEVO con I/O de bajo nivel esta bloqueado por el SANDBOX
'   de Excel para Mac (ni GetSaveAsFilename ni GrantAccessToMultipleFiles lo habilitan para Open).
'   El metodo que SIEMPRE funciono en Mac es SaveAs FileFormat:=62 (xlCSVUTF8): Excel guarda SU
'   PROPIO archivo y el sandbox le concede la ruta elegida en GetSaveAsFilename.
'   Se CONSERVA el build canonico de v15 (expansion de combos), pero en vez de armar el string y
...
'   El parser tolera BOM (lo strippea: bulk-upload.js `replace(/^<BOM>/,'')`), cualquier fin de
'   linea (parseCSV consume \r/\n/\r\n) y filas cortas (`(row[i]||'')`). Ver secciones 6-8.
...
'     - "Estatus"          -> "Archivado" + "Validacion"            (V/F)
'     - "Forzar duplicado" -> "Forzar" + "Archivar anterior"        (V/F)
'     - "Productos"        -> 3 grupos Producto/Precio/Cantidad/Unidad via CAT_Productos
...
'   Metadata (modo/empresa/quoteName/notas/asignado/valida) se lee LABEL-DRIVEN (robusto a
...
'   1=Estatus  2=Forzar duplicado  3=Id SH  4=Cliente  5=Numero de parte ...
'
...
' La version que se anuncia al terminar se LEE del titulo de la hoja (A2 =
' "Carga Masiva Steelhead v13"), no se escribe a mano. Hasta el 2026-08-05 el aviso decia
' "CSV v12 exportado" dentro de la plantilla v13: un numero a mano se desfasa en el
' siguiente cambio de layout y miente justo cuando el operador revisa que exporto.
' Leyendolo del titulo queda atado a lo que el MISMO ve en la hoja.
' Devuelve "v13 " (con el espacio) o "" si el titulo no trae version -- sin dato no se
...
Sub ExportarCSV()
    ...
    ' Varios clientes en COTIZACION+NP: SOPORTADO -- el applet crea UNA cotizacion por
    ' cliente, nombrada "{Cliente} {Layout}". Hasta el 2026-08-05 esto se BLOQUEABA aqui
    ...
    ' plantilla vetaba lo que el applet si sabe hacer.
    ...
    If Not esCotizacion Then
        Dim nData As Long
        For r = DATA_START To lastR
            ' Fila con datos = PN o Id SH (igual que el volcado sec.6 y el parser downstream)
    ...
    ' -- 3) Plan de columnas canonicas (header-driven sobre la fila 7) --
    ...
    For k = 1 To n
        ...
        ' (Cliente y PN vacios) salgan deterministas en vez de en orden arbitrario.
    ...
    ' -- 6) Construir matriz de salida (metadata + header canonico + filas) --
    ' Se vuelca a un libro temporal y se exporta con SaveAs FileFormat:=62 (ver sec.8).
    ' Las celdas van como TEXTO ("@") para preservar punto decimal, ceros a la izquierda y
    ' fechas tal cual; Excel hace el escapado CSV (comas/comillas) -- igual semantica que el
...
NextK:
    ...
    ' la ruta que el usuario eligio en GetSaveAsFilename (a diferencia de Open de bajo nivel,
    ...
    ' ExportarCSV NO escribe en la plantilla: solo lee Upload y CAT_Productos, y vuelca a un
    ' libro NUEVO. Aun asi se desprotege aqui porque el libro se entrega con la ESTRUCTURA
    ' protegida (workbookProtection lockStructure="1"), y esta es la unica parte de la macro
    ' que crea y manipula un libro. Cuesta dos llamadas y quita la duda; si ExportarCSV
    ' siguiera fallando con la plantilla protegida, el culpable NO esta en esta macro.
...
' Una fila cuenta como "con datos" si trae Numero de parte O Id SH. Mismo criterio que el
' parser downstream (bulk-upload.js: `if (!pn && !idShEarly) continue`). El Id SH identifica
' un PN EXISTENTE en Steelhead, asi que una fila solo-IdSH (sin PN) es valida y debe exportarse.
...
Private Function MetaVal(ws As Worksheet, ByVal labelKey As String) As String
    ...
    ' y devuelve el primer valor no vacio a la derecha (1..4) que NO sea otro label.
