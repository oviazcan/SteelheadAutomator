...
'   COTIZACION+NP ya no bloquean el export (confirmacion en vez de veto) y la version del
'   aviso final se lee del titulo de la hoja (TplVer) en vez de estar escrita a mano.
...
'   (lastR = max(lastPN, lastID)) pero el volcado filtraba SOLO por PN -> 0 filas. Ahora el
'   criterio es PN <> "" OR Id SH <> "" (helper HasRowData) en el aviso >2000 y el volcado;
'   la clave de orden desempata por Id SH; el MsgBox reporta nOut (filas exportadas) en vez de n.
'   Estructura de escritura INTACTA (ADODB.Stream WriteUtf8NoBom, Excel 2019 Windows).
' V15 (2026-06-09): reescrito para el spreadsheet v12 (hoja "Upload", 60 cols visibles).
'   CAMBIO DE FONDO vs v14: ya NO usa SaveAs (dump crudo). Construye el CSV CANONICO
'   en memoria, expandiendo 3 columnas combo a su forma que el parser entiende:
...
'   Orden determinista por (Cliente, PN) con quicksort en memoria (sin libro temporal).
'
...
Sub ExportarCSV()
    ...
    ' ?? 6) Construir CSV (bloque metadata + header canonico + filas) ??
    Dim sb As String
    sb = CsvQuote(modoRaw) & vbCrLf
    sb = sb & "Empresa Emisora:," & CsvQuote(empresa) & vbCrLf
    sb = sb & "Nombre Cotizacion/Layout:," & CsvQuote(quoteName) & vbCrLf
    sb = sb & "Notas Externas:," & CsvQuote(notasExt) & vbCrLf
    sb = sb & "Notas Internas:," & CsvQuote(notasInt) & vbCrLf
    sb = sb & "Asignado:," & CsvQuote(asignado) & vbCrLf
    sb = sb & "Valida Hasta (dias):," & CsvQuote(validaDias) & vbCrLf
    sb = sb & BuildLine(outHdr) & vbCrLf

    Dim i As Long, ri As Long, t As Long, col As Long, sv As String, fz As String, line As String
...
Private Sub WriteUtf8NoBom(ByVal path As String, ByVal content As String)
    ...
    st.Position = 0: st.Type = 1: st.Position = 3      ' saltar BOM utf-8 (3 bytes)
    Set bin = CreateObject("ADODB.Stream")
    bin.Type = 1: bin.Open
    st.CopyTo bin: st.Close
    bin.SaveToFile path, 2: bin.Close                  ' 2 = sobrescribe
End Sub
