...
' Preserva formulas (HasFormula skip -> Planta Schneider, Proceso, Esp.Spec, Longitud, predictivos).
'
' V20 (2026-08-07): la plantilla se entrega PROTEGIDA (solo se pueden editar los campos de
'   captura), y esta macro es la que mas escribe de todas: ClearContents, valores, defaults,
'   placeholders y un Copy/PasteSpecial por columna calculada. Con la hoja protegida truena
'   con error 1004 en la PRIMERA celda. Ahora desprotege al entrar y reprotege al salir via
'   ModProteccion (que repone el candado EXACTO que traia el archivo, no uno inventado).
'   La reproteccion va en una etiqueta `Cleanup` a la que se llega TAMBIEN por error: si la
'   macro revienta a media pasada, lo peor que puede pasar es que el archivo se quede sin
'   candado y nadie se entere hasta que alguien borre una formula por accidente.
'   El aviso de "Proceso sin formula" se movio DESPUES de reproteger -- un MsgBox que espera
'   respuesta humana con la plantilla abierta de par en par es una ventana de riesgo gratis.
'
' V19 (2026-08-03): CORRIGE EL TOPE del v18. La plantilla v13 final trae una columna MAS de
'   la que el v18 supuso: "Instrucciones de Empaque" (60=BH), agregada despues de los racks,
'   asi que la hoja mide 67 columnas (A..BO) y no 66. Con el tope en 66, la ULTIMA columna
'   (67=BO Tiempo de Entrega) nunca se limpiaba: conservaba el valor de la carga anterior y se
'   volvia a subir en la siguiente, con la plantilla viendose limpia. Los 3 loops pasan a 67.
'   Los indices de placeholders NO cambian: phCols/phHybridCols/boolCols apuntan todos a
'   columnas <= 45, o sea ANTES de la columna nueva. Instrucciones de Empaque es texto libre
'   (como Notas adicionales): se limpia y se queda vacia, sin placeholder ni formula.
'   LECCION: este modulo se rompe cada vez que la hoja cambia de ancho, y en silencio. El tope
'   esta atado a la plantilla real por tools/test/module5-column-cap.test.js -- si vuelven a
...
'   3 pares de rack (Rack 3/4/5 + sus Pzas) DESPUES del par 2, lo que corre +6 todo lo
'   posterior. Este modulo NO se auto-ajusta al insertar columnas en Excel porque
'   referencia columnas por INDICE NUMERICO -- de ahi que haya que tocarlo a mano:
'     - "For c = 1 To 60" -> "1 To 67" (3 lugares: limpieza, restauracion de formulas y
'       LimpiarEspacios). Sin esto las ultimas columnas (Empaque..Tiempo de Entrega se
'       corrieron a 60..67) NUNCA se limpian -> datos de la carga anterior sobreviven a
'       LimpiarDatos y se re-suben en la siguiente. Es el modo de falla mas caro de los
'       cuatro porque no se ve: la plantilla "se ve limpia".
'     - phCols suma los 3 racks nuevos (39, 41, 43) para que reciban "(seleccione)".
'     - phHybridCols: Tipo de Geometria paso de 39 a 45. Si no se actualiza, el
'       placeholder "(seleccione o escriba)" se escribiria DENTRO de la columna Rack 3
'       (que en v13 ocupa el indice 39) y Tipo de Geometria se quedaria sin placeholder.
...
'
' V17 (2026-06-12): restaura TODAS las columnas calculadas (no solo Proceso) copiandolas
'   desde la FILA MODELO (lastRow). Generico: detecta que columnas tienen formula ahi y las
...
'   5=E  Numero de parte (Texto)  6=F  Descripcion (Texto)
'   7=G  PN alterno (Texto)       8=H  Grupo (Hibrido -> "(seleccione o escriba)")
'   9=I  Cantidad (# = 1)        10=J  Precio ($)
'  11=K  Unidad precio (Drop)    12=L  Divisa (Drop, USD)
'  13=M  Precio default (V/F)    14=N  Linea (Drop)
'  15=O  Metal base (Hibrido)    16=P  Etiqueta 1 (Drop)  17=Q Etiqueta 2  18=R Etiqueta 3  19=S Etiqueta 4
'  20=T  Planta Schneider (Calc) 21=U  Proceso (Formula)
'  22=V  Productos (Drop)
'  23=W  Spec 1 (Drop)           24=X  Esp.Spec1 (Calc)   25=Y Spec2  26=Z Esp2  27=AA Spec3  28=AB Esp3  29=AC Spec4  30=AD Esp4
'  31=AE KGM (#)  32=AF CMK (#)  33=AG LM (#)  34=AH Min Pzas Lote (#)
...
'  45=AS Tipo de Geometria (Hibrido)
'  46=AT Longitud (Calc)  47=AU Ancho (#)  48=AV Alto (#)  49=AW Diam.Ext (#)  50=AX Diam.Int (#)
'  51=AY Plata..59=BG Epox.MTR (Calc, predictivos)
...
Sub LimpiarDatos()
    ...
    lastPN = wsUp.Cells(wsUp.Rows.Count, 5).End(xlUp).Row  ' col E = Numero de parte
    ...
    ' A partir de aqui se ESCRIBE: hoja desprotegida y salida garantizada por Cleanup.
    ...
    ' (Si en el futuro vuelven, agregar sus indices aqui.)
...
NextCell:
    ...
    '   4 Cliente | 11 Unid Precio | 14 Linea | 16-19 Etiq1-4 | 22 Productos
    ...
    ' Hibridas -> "(seleccione o escriba)": 8 Grupo | 15 Metal base | 45 Tipo de Geometria
    ' V13: Tipo de Geometria paso de 39 a 45 (los 3 pares de rack nuevos la empujaron).
    Dim phHybridCols As Variant
    phHybridCols = Array(8, 15, 45)

    Dim phc As Variant
    For r = 9 To lastRow
        ' Defaults especiales de los combos de parametros + Divisa + Cantidad
        ...
        ' Cantidad (col 9 = I): 1 constante. Forzado (no SetIfEmpty) salvo formula.
    ...
    ' Restaurar TODAS las formulas calculadas (Proceso col 21=U, Esp.Spec 24/26/28/30,
    ' predictivos 51..59, Planta Schneider 20, etc.) copiandolas desde la FILA MODELO
    ' (lastRow, normalmente 508 -- casi nunca se toca a mano). Generico: detecta que columnas
    ' tienen formula en la fila modelo y restaura esas hacia toda la zona de datos.
...
Cleanup:
    ' Se llega aqui por el camino normal Y por error. Reproteger es lo PRIMERO: el resto
...
' Helper: setea un default solo si la celda esta vacia y no tiene formula.
...
' Limpia espacios iniciales, finales y dobles en celdas de texto. NO borra datos ni toca formulas.
'
' V16 (2026-08-07): desprotege al entrar y reprotege al salir (ModProteccion), igual que
'   LimpiarDatos. Escribe poco --solo las celdas cuyo texto cambia-- pero basta UNA para que
'   la hoja protegida la mate con error 1004, y el operador la corre justo cuando el CSV ya
'   le fallo por espacios de mas.
' V15 (2026-08-03): tope 1..67 (el v14 decia 66 y dejaba fuera Tiempo de Entrega).
' V14 (2026-08-03): layout v13 -> loop 1..66 (antes 1..60). Sin esto, las ultimas columnas
'   (Notas adicionales..Tiempo de Entrega) quedarian fuera de la limpieza de espacios.
...
Sub LimpiarEspacios()
    ...
    ' A partir de aqui se ESCRIBE: hoja desprotegida y salida garantizada por Cleanup2.
