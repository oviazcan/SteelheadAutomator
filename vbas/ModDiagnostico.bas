...
' === MÓDULO DE DIAGNÓSTICO (2026-08-08) ===
' Módulo SUELTO: se importa, se corre y se puede borrar. No lo llama nadie y no cambia el
' comportamiento de la plantilla.
'
' Existe porque el sombreado "no reacciona" y llevamos tres intentos de arreglarlo por
' hipótesis. Las causas posibles producen el MISMO síntoma visible y sólo se distinguen
' midiendo:
'
'   A. Los eventos de Excel están apagados  -> Worksheet_Change ni siquiera corre.
'   B. El handler Worksheet_Change no está en la hoja Upload, o no llama a la macro.
'   C. UserInterfaceOnly no está activo     -> la macro corre y truena al pintar.
'   D. La macro corre y pinta, pero tarda   -> es un problema de rendimiento.
'   E. La macro corre, pinta y es rápida    -> es un problema de REDIBUJO de pantalla.
'
' SA_PorQueNoSombrea las separa. La prueba clave es la (7): escribe H1 desde VBA y mira si el
' color cambia SOLO. Si no cambia, el evento no está corriendo y no tiene caso seguir tocando
' la macro — el problema está en el handler o en los eventos.
...
Public Sub SA_PorQueNoSombrea()
    ...
    ' Si no cambia, el problema NO está en la macro de sombreado.
    ...
    ' ── 8) ¿Cuánto tarda la macro llamada a mano? ──
    t0 = Timer
    On Error Resume Next
    SombrearModoSoloPN
    On Error GoTo 0
    ...
    ' ── 9) ¿El color quedó acorde al modo? ──
...
' Botón de pánico. Devuelve Excel a un estado sano y vuelve a sincronizar el sombreado.
' Sirve sobre todo cuando los eventos quedaron apagados: eso mata Worksheet_Change para TODA
' la sesión y no hay nada en la pantalla que lo delate — las cosas simplemente "ya no hacen".
