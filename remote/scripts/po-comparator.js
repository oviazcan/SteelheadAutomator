  ...
  async function processOneFile(file, fileIndex, totalFiles) {
    ...
    if (searchResult.match === 'exact') {
      ...
      try {
        ...
        candidates = await window.OVOperations.findCandidateOVs(pdfData, customerId);
      ...
      if (candidates.length > 0) {
        ...
        const selection = await window.OVOperations.showCandidateSelector(candidates, pdfData);
        ...
        if (selection.action === 'adopt') {
          ...
          try {
            ...
            orderId = await window.OVOperations.adoptExistingOV(selection.candidate, pdfData, file);
          ...
          try {
            ...
            creationData = await window.OVOperations.fetchCreationData(customerId);
          ...
          const formData = await window.OVOperations.showCreationWizard(pdfData, creationData, customerId);
          ...
          try {
            ...
            orderId = await window.OVOperations.createNewOV(formData, pdfData, file);
        ...
        const manualChoice = await window.OVOperations.showNoMatchOptions(pdfData);
