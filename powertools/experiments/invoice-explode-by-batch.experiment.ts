...
const formatearNO = (descripcionNP: string | null, observaciones: string[]): string => {
...
const construirDescripcionCFDI = (p: {
  ...
  descripcionNP: string | null       // partNumber.description — texto de la pieza, va en la sección N: (flag MostrarNP)
  acabados: string[]
...
// ═════════════════════════════════════════════════════════════════════════════
// 🧪 EXPERIMENTO — EXPLOTAR POR LOTE (CERRADO — NO FUNCIONA — NO PUBLICAR)
// ─────────────────────────────────────────────────────────────────────────────
// RESULTADO (2026-06-16, OV/factura de prueba 1096, SOLI 3374909, 2 lotes):
//   ❌ NO funciona. El hook SÍ pintó 2 líneas con el mismo salesOrderLineItemId y
//   las cantidades fueron correctas (5pz→11.1158 LBR, 1pz→2.2231 LBR, suman el
//   total — invariante OK). PERO la asociación pieza↔línea ("Part Locations") la
//   determina el SOLI, no la línea: ambas líneas heredaron LAS DOS partidas (las 6
//   piezas), disparando "Done invoicing quantity > line item quantity". El hook no
//   tiene canal para repartir partAccounts entre líneas que comparten SOLI.
//   ⇒ Para facturar por lote, la función de domain "cada lote en su propia línea
//      de OV" es el ÚNICO camino (sube la granularidad al SOLI). Detalle completo
//      en docs/applets/powertools-facturacion.md (§ Hallazgo 2026-06-16).
//   Se conserva como artefacto del hallazgo; jamás se publica a productivo.
// ─────────────────────────────────────────────────────────────────────────────
// Esta es una COPIA del hook productivo `invoice.ts` con un solo cambio: cuando
// EXPLOTAR_POR_LOTE === true y un SOLI agrupa >1 lote (receivedBatch), en vez de
// UNA línea con la cantidad sumada, emite UNA LÍNEA POR LOTE.
//
// El objetivo es PROBAR si Steelhead acepta múltiples lowCodeDefaultInvoiceLineItems
// que comparten el mismo `salesOrderLineItemId` (el binding con la OV). Hoy NO
// sabemos si Steelhead (a) las persiste todas, (b) dedup por id, (c) suma/sobre-
// escribe, o (d) rechaza al guardar. Este hook lo deja ver en el PREVIEW.
//
// PASOS:
//  1. Pega TODO este archivo en el editor de Power Tools (slot `invoice`) sobre
//     la factura de prueba (OV 1096, 2 lotes). NO le des "publicar/guardar" todavía.
//  2. Observa el PREVIEW: ¿pinta 2 líneas (una por lote) con cantidades que suman
//     el total? Lee los mensajes `🧪 EXPERIMENTO …`.
//  3. Si el preview se ve bien y quieres probar la PERSISTENCIA: guarda la factura
//     de prueba y revisa si el SOLI queda "facturado completo" o si truena / dedup.
//  4. Pase lo que pase, REVIERTE el slot a la versión productiva:
//     `python3 tools/lowcode_sync.py pull invoice`  (o push del invoice.ts bueno).
//
// La cantidad por lote usa la MISMA fórmula que el cálculo productivo
// (pa.quantity × conversionFactor), así que la suma de las líneas explotadas
// debe igualar bit-a-bit lo que hoy se factura en una sola línea.
// ═════════════════════════════════════════════════════════════════════════════
const EXPLOTAR_POR_LOTE = true // ← experimento. Productivo iría en false / sin esto.
...
const getInvoicePricing = (
  ...
  // 🧪 EXPERIMENTO: explotar y consolidar son opuestos; si explotamos, no consolidamos.
  const consolidarPorProducto =
    !EXPLOTAR_POR_LOTE && sos.length > 0 && sosConFlag === sos.length
  if (!EXPLOTAR_POR_LOTE && sosConFlag > 0 && sosConFlag < sos.length) {
    helpers.addErrorMessage({
      severity: 'warning',
  ...
  inputs.uiInvoiceLineItems.forEach(uiLine => {
    ...
    // --- 6️⃣ 🧪 EXPERIMENTO: desglose por lote ---
    // Agrupa partAccounts por receivedBatch.id (cruzando todos los pnwo de la
    // línea) con la MISMA conversión de unidad del paso 3, para que la suma de
    // las líneas explotadas iguale la cantidad total de la línea (invariante).
    const lotesBuckets = new Map<
      string,
      { batchName: string | null; qty: number; packingSlips: Set<string> }
    >()
    for (const pnwo of partNumbersOnThisLine) {
      const conversionFactor =
        pnwo.partNumber?.partNumberUnitConversions?.find(c => c.unit?.id === lineUnit)?.factor ?? 1
      for (const pa of pnwo.partAccounts ?? []) {
        if (!pa) continue
        const batch = pa.receivedBatch
        const key = batch ? `b${batch.id}` : '__sin_lote__'
        if (!lotesBuckets.has(key)) {
          lotesBuckets.set(key, { batchName: batch?.name ?? null, qty: 0, packingSlips: new Set() })
        }
        const bucket = lotesBuckets.get(key)!
        bucket.qty += (pa.quantity ?? 0) * conversionFactor
        const ps = batch?.customInputs?.DatosRecibo?.PackingSlip
        if (ps != null && String(ps).trim() !== '') bucket.packingSlips.add(String(ps))
      }
    }

    const debeExplotar = EXPLOTAR_POR_LOTE && !loteMinimoCargado && lotesBuckets.size > 1
    if (debeExplotar) {
      let qtyReconstruida = 0
      for (const [, bucket] of lotesBuckets) {
        const descBase = construirDescripcionCFDI({
          partNumberName,
          descripcionNP,
          acabados,
          nombreProducto,
          ...
          nombresLotes: bucket.batchName ? [bucket.batchName] : [],
          packingSlips: [...bucket.packingSlips],
          loteMinimoCargado: false,
          piezasLoteMinimo: null,
          flags,
        })
        // Prefijo [Lote X] visible para distinguir las líneas en el preview.
        result.lowCodeDefaultInvoiceLineItems.push({
          salesOrderLineItemId: uiLine.salesOrderLineItemId, // ← MISMO id en todas (el punto del experimento)
          taxCodeId: uiLine.taxCodeId ?? null,
          quantity: bucket.qty,
          rate: uiLine.rate,
          productId: uiLine.productId,
          description: `[Lote ${bucket.batchName ?? 's/lote'}] ${descBase}`,
        })
        qtyReconstruida += bucket.qty
      }
      helpers.addErrorMessage({
        severity: 'info',
        message: `🧪 EXPERIMENTO: SOLI ${uiLine.salesOrderLineItemId} → ${lotesBuckets.size} líneas por lote (qty total ${uiLine.quantity} ⇒ reconstruida ${qtyReconstruida}; rate ${uiLine.rate}). Todas comparten el MISMO salesOrderLineItemId.`,
      })
      return // no push normal ni lineMetadata para esta línea
    }

    // --- 6️⃣ Construir descripción CFDI (caso normal / SOLI de 1 lote) ---
  ...
  if (consolidarPorProducto && lineMetadata.length > 0) {
    ...
    for (const [, grupo] of grupos) {
      ...
      // sub-tabla del PDF (lineasConsolidadasPorProducto). Se envuelve en
      // "N: N/A O: …" con el mismo orden de O: que el caso normal:
      // Producto, Acabado, OC, Lote, OT, PS.
      const partes: string[] = []
      ...
      // Consolidado sin descNP → sección N: N/A; las observaciones van en O:.
      const descConsolidada = formatearNO(null, partes)
