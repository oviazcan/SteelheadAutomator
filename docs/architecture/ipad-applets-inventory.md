...
> sólo vale si el flujo core ES la descarga, y aquí no lo es. **Lo nuevo es que tampoco era un "sí"
> simple:** declara `pdf.js` + `pdf-lib` (902 KB, +50% del bundle) y el worker de `pdf.js` **se baja
> en runtime**, o sea **código remoto prohibido por la Guideline 2.5.2** — la razón misma de que este
> bundle sea estático.
>
> De ahí sale una **tercera vía**, con mecanismo propio (`excludeScripts` en `bundle.json`, soportado
> por `build-safari.sh`): **integrar el applet SIN sus librerías pesadas**. Se pierden sólo la
> impresión combinada y las miniaturas de PDF; el flujo core funciona. Coste real **+63 KB (+3.5%)**
> en vez de +962 KB. Requisito: **el applet debe degradar solo** cuando la librería no está (aquí
> `hayPdfLib()`/`hayPdfJs()`); el build no lo valida.
>
> Confirmado con el operador que **sí se mandan remisiones desde el iPad**, así que el valor de piso
> está verificado, no supuesto. **Pendiente: recompilar en Xcode.**

> **Actualización 2026-07-27 — Bundle v0.6.0 (+3) y CRITERIO CORREGIDO sobre "descarga".** Entraron **`wo-schedule-button`** (readout inline de programación en la ficha de OT — este applet **nació POR el iPad**: ahí la tarjeta Cliente que trae el 📅 nativo se colapsa, y aun así seguía fuera del bundle), **`wo-listing-columns`** (columnas PN/Programación/Lote en el listado de OTs) y **`po-listing-filters`** (buscador global de OC; autoInject, sin lanzador). Lanzadores nuevos: `open-wo-schedule`, `toggle-wo-pn-column`, `toggle-wo-sched-column` → **32 applets, 11 lanzadores**, **1.41 MB**. **El criterio de descarga estaba mal aplicado:** el scanner marca NO-APLICA en cuanto ve `a.download`, pero en estos dos la descarga es **la generación del PDF — función lateral y opt-in**, no el flujo core. *NO-APLICA sólo vale cuando el FLUJO CORE del applet ES la descarga* (auditor, carga-masiva, file-uploader, portal-importer, po-reconciler, spec-migrator, spec-params-bulk, process-canon); si es una función más, el applet SÍ va al bundle y en iOS simplemente esa parte no opera. Con el criterio anterior el iPad se quedaba sin lo más útil de piso por una función secundaria. Siguen fuera por valor (no por técnica): `hash-scanner` (dev-tool), `inventory-reset`/`po-comparator`/`report-liberator` (oficina/admin, dependen de subir archivos o portapapeles). **Pendiente: recompilar en Xcode.**

> **Actualización 2026-07-23 — Bundle v0.5.9 (+2 auto-inyectados de piso).** Entraron al bundle **`batch-name-filter`** (Filtrar Lote por Nombre — box inline en el header del Panel de Envío `/Domains/<id>/Shipping`) y **`schedule-batch-highlighter`** (Resaltar Lote en Programación — buscador inline en la barra de filtros del Schedule Board). Ambos **`autoInject:true`, SIN lanzador de popup** (se activan solos por su gate/FAB inline), y ambos **VALIDADOS en vivo** (2026-07-22/23). Se integraron por su valor en piso (operadores filtrando/resaltando lotes en envío y programación). Total: **29 applets, 8 lanzadores** (sin cambio en el canal de lanzadores). Peso del bundle concatenado: **~1.24 MB**. Descartados de esta pasada: `hash-scanner` (dev-tool), `inventory-reset`/`po-comparator`/`report-liberator` (admin/oficina que dependen de subir archivos/portapapeles, no de piso) y los 8 con bloqueador de descarga (`a.download` no opera en iOS). **Pendiente: recompilar en Xcode** (el bundle es estático).
...
| batch-name-filter | directo | alto | bajo | steelhead-api, batch-name-filter-core | ninguna | **en bundle v0.5.9**; box inline en el Panel de Envío; autoInject, sin lanzador; validado en vivo 2026-07-22 |
| schedule-batch-highlighter | directo | alto | bajo | schedule-batch-highlighter-core | ninguna | **en bundle v0.5.9**; buscador inline en el Schedule Board; 100% DOM (no usa steelhead-api); autoInject, sin lanzador; validado en vivo 2026-07-23 |
