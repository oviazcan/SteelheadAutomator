...
> **Actualización 2026-06-30 — Bundle v0.4.0 (canal de lanzadores del popup + auto-router).** Se resolvió la infraestructura de "applets con interfaz" en Safari sin tocar el código de los applets: el popup ofrece **botones lanzadores** que escriben un comando en `browser.storage.local` (`saCommand:{action,nonce}`); `bridge.js` (mundo aislado) lo reenvía al MAIN world por `postMessage`; y un nuevo `safari/sa-dispatcher.js` (MAIN world, concatenado por `build-safari.sh`) resuelve la acción → función global del applet vía **allowlist** (`LAUNCH_FN`) + fallback a `config.actions[].fn`. Con esto **entraron al bundle**: `vale-almacen` (FAB, "directo"), y las 4 "con-popup" **`archiver`, `sensor-status-autofill`, `load-calculator`, `auto-router`** (lanzadas desde el popup; auto-router además con su FAB). Total: **21 applets**. Regresión en `tools/test/build-safari.test.js` (canal popup→bridge→dispatcher→applet consistente, 6 lanzadores). **Gotcha auto-router:** en Chrome su trigger de popup (`chrome.runtime.onMessage`) está muerto en MAIN world y solo corre por el FAB; en Safari el dispatcher usa `postMessage`, así que los lanzadores **sí operan** (v0.3.0 lo dejó pendiente; v0.4.0 lo revive). Nota de peso: el bundle pasó de ~300 KB a **~870 KB** (mayormente `load-calculator` + `auto-router`) — perfilar en iPads A12 o anteriores.
