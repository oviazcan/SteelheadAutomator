...
**`Module5_v19` ya pegado en ambas plantillas**, verificado extrayendo el VBA con `olevba -c` (3× `For c = 1 To 67`); el v18 se borró del repo para que nadie pegue el equivocado. **Guía para el EQUIPO** (la que se difunde): [`docs/training/guia-plantilla-v13.html`](../training/guia-plantilla-v13.html) — **se GENERA** con `tools/build-guia-v13.js` leyendo el `.xlsm` real, no se edita a mano · nota técnica: [`docs/plantilla-carga-masiva-v13.html`](../plantilla-carga-masiva-v13.html) · contrato: [`docs/applets/bulk-upload-v13.md`](bulk-upload-v13.md).

**2026-08-05 — LAS TRES ERRATAS DE VERSIÓN, corregidas y VIVAS.** Las tres decían un número **escrito a mano** y por eso las tres se desfasaron al pasar a v13; el arreglo fue **quitarlo**, no corregirlo: (a) el MsgBox de `ExportarCSV` decía `"CSV v12 exportado"` → `TplVer(ws)` lo lee del título de la hoja (`A2`); (b) el panel decía `Steelhead Automator v10` fijo → ahora muestra la versión que el parser **DETECTÓ** (`schemaVersion`), con lo que deja de ser un rótulo y contesta *«¿con qué layout se leyó mi archivo?»* (deploy **v1.11.84**); (c) la hoja **`Ayuda`** de AMBOS `.xlsm` describía el layout **v10** completo —69 columnas `A–BQ`, racks ×2 en `AP–AS`, "Etiqueta 5"—: **no era un rótulo, era un mapa de columnas equivocado dentro del archivo que el operador abre**. Además **se abrió el multi-cliente** en `COTIZACIÓN+NP`: `ExportarCSV` lo **bloqueaba** mientras el applet crea una cotización por cliente desde hace versiones — capacidad viva e inalcanzable desde el flujo normal; ahora es confirmación (`Module1 v15.5`, ambos `.bas`). Verificado **bajando los archivos del sitio**: zip íntegro, 4 macros, `3× For c = 1 To 67`, `"Varios clientes"` en el código, `TplVer(ws)`, hoja `Ayuda` de 136 filas idéntica al texto generado del layout real y `verify-template-layout` 10/10.

**⛔ El `.xlsm` NO se edita por XML, tampoco "solo una hoja".** Reescribir `sheet4.xml` produjo archivos que **Excel abre como dañados**, y pasaron SEIS verificaciones antes de que se notara (zip íntegro, XML válido, `vbaProject.bin` byte a byte, 1 entrada de 33, macros intactas, `verify-template-layout` 10/10): ninguna contestaba *«¿Excel lo acepta?»*. Causa: tocar el `<pane>` (`topLeftCell="A1"` con `ySplit="1"` es imposible), **fuera de `<sheetData>`, que era lo único que había que tocar**. Vía vigente: `tools/rebuild-ayuda-sheet.js <archivo> --text` emite el contenido para pegarlo en Excel. **Al pegar, limpiar el rango completo primero**: el contenido nuevo era más corto (160 líneas vs 176) y en una de las dos plantillas sobrevivieron las filas 161–176 del texto viejo —con el correo anterior y la mención a "Etiqueta 5"—; se cazó contando filas con texto contra las esperadas, porque el principio de la hoja se veía perfecto.

**Lector de `.xlsm` compartido: [`tools/lib/xlsx-read.js`](../../tools/lib/xlsx-read.js)** (trinquete `tools/test/xlsx-read.test.js`, muerde: con el lector viejo falla 3 de 6). Nació de un bug que mordió **dos veces el mismo día en el mismo archivo**: un regex que exige la etiqueta de cierre **se traga los elementos self-closing y le cuelga al anterior el contenido del siguiente** — primero con las celdas (`<c r="C3" s="109"/>` corrió toda la fila 3 y se leyó mal el encabezado del layout), después con las validaciones (`<dataValidation sqref="E8"/>` vacía hizo reportar que la columna `B` no tenía desplegable cuando `B9:B508` sí lo declara). **Corregir el patrón en un sitio no lo corrige en los demás.**
