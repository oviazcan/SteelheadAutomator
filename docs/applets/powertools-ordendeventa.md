...
> **Estado (2026-06-06):** ✅ **LIVE en Steelhead** — pusheado vía `tools/lowcode_sync.py push received-order` (mutation `CreateReceivedOrderLowCode`), nueva versión activa **`id=1873`** (2026-06-06T20:55Z, by OMAR VIAZCAN). La versión previa `1857` (código viejo sin validación) queda en historial; rollback = `lowcode_sync.py pull --all-versions` + re-push del `.ts` de 1857. Verificado byte-exacto con `lowcode_sync.py diff received-order` → "OK — local == server activo (id=1873)". **Pendiente: Phase-0** (confirmar en el Test panel del Power Tool que `partNumber.partNumberLabels` llega poblado como input en "Add Parts to Sales Order"). Si viniera vacío, TODOS los NP Schneider saldrían con chip rojo falso "sin etiqueta de planta" (advisory, no bloquea Save) → rollback inmediato y mover la validación a un applet de extensión con GraphQL. Nota: el fix de config `SQR→SQ1` también está live (gh-pages 1.6.39); independiente de este push.
