...
**Guarda de regresión:** `tools/wo_label_consolidation.mjs` + `tools/wo_label_consolidation.test.mjs` (8 tests, incluye OT 5103 y OT 262). Compila con `tsc --target es2017` vía `lowcode_sync.py push --dry-run` (17408 TS → 12552 JS, sin errores).

**VALIDADO EN VIVO (OT 5848, 2026-06-08, versión activa `id=10642`):** un diagnóstico temporal gated a la WO (volcado de `receivedBatches` + `allPartsOnWorkOrder` vía `addErrorMessage` info) confirmó el shape real: `receivedBatches(1)=[b#1279733(nc=25)]`, `allPartsOnWorkOrder(2)=[pn#3369273/g#null/b#1279733/q90 || pn#3369273/g#null/b#1279733/q210]`. Es **1 lote con 2 partAccounts del MISMO `partNumber.id` (3369273)** y `partGroup` **null** en ambos — la agrupación por `partNumber.id` los colapsa correctamente. El hook ahora reporta `✅ Generated 25 PDF entries across 1 batch(es)` (antes 50), con cantidad consolidada 90+210=300. **Gotcha de propagación:** el primer PDF post-deploy salió con el comportamiento viejo (50 págs) porque Steelhead cacheó/propagó la versión activa con retraso; la reimpresión ya salió correcta. El diagnóstico se quitó y se redeployó la versión limpia (`id=10642`). Historial: `10639` (fix), `10640` (fix+diag), `10642` (fix limpio). Rollback a pre-fix = `10474`.
