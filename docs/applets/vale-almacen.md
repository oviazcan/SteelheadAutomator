...
**Gate del FAB (VIVO 1.7.200, 2026-07-24):** el FAB 📦 se **quitó del LISTADO** `/Domains/<d>/WorkOrders` (dashboard general muy cargado — decisión del usuario). `ALLOWED_PATH_RE` ahora matchea `WorkOrders` SOLO con `/<id>` (la ficha), + Workboards/Maintenance/Sensors/Inventory como antes. Además NO carga en pestañas de impresión (`isPrintTab()` = `?sa_print=`). Mismo cambio en `paros-linea` (sin bitácora propia; gate `Workboards|WorkOrders/<id>`). Commit `f786ffe`/`df511fd`.
