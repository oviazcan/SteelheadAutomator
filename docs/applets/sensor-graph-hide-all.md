...
**Nota de alcance (2026-08-03) — CORREGIDA el mismo día:** primero se concluyó que el dominio no tenía Sensor Dashboards porque `/Domains/344/SensorDashboards` salía vacío. **Era la ruta equivocada**: la real es `/Domains/<d>/Maintenance/SensorDashboards/<id>` (p.ej. la 117) y sí existen. Con el HTML que aportó el operador se midieron **`VisibilityIcon` y `VisibilityOffIcon`**. Su deuda real no es el path sino que ancla primero a un `aria-label` EXACTO **en inglés** con el `data-testid` como único fallback. **Verificado en el DOM real: los dos aria SIGUEN EN INGLÉS** (`'Hide this sensor in the graph.'` / `'Show this sensor in the graph.'`), o sea la primaria funciona hoy — y ahora, además, detrás hay un fallback por FORMA del icono, así que ya no se queda sin red si SH los traduce. **Ojo: la correspondencia icono↔acción es INVERSA** — el ojo TACHADO va en el botón «Show this sensor…» (está oculto, ofrece mostrarlo).
