...
**Estado (2026-07-20):** **ACTIVO en producción** — `SOLO_PRECIO_FASTPATH_ENABLED=true`, `bulk-upload.js`
1.5.41 + `bulk-upload-parse.js`, config **1.7.163**, firmados KMS, gh-pages sincronizado (verificado
EN VIVO). El flag se activó por decisión del operador (1.5.40 se deployó primero con flag OFF en
1.7.162; 1.5.41 lo prendió en 1.7.163). **A observar en la primera corrida solo-precio real:** que el
panel muestre "STEP 6 enrich omitido (fast-path SOLO_PRECIO)" y que el precio + default se apliquen
bien. **Para desactivar** si algo sale mal: poner el flag en `false` + re-deploy (`wb-deploy.sh`).
El atajo solo se activa cuando `classifyRunIntent` da exactamente `SOLO_PRECIO` (todos existentes,
precio, cero enriquecimiento); una sola fila con enrich o un PN nuevo lo desactiva (invariante testeado).
...
- ❌ **F4 Task 3 (extraer las decisiones a `bulk-upload-build.js`): DESCARTADO (2026-07-23, decisión del usuario).** El módulo `remote/scripts/bulk-upload-build.js` (funciones puras `decideLabelIds`/`decideDims`/`decideDimValueIds`/`decideOptInOuts`/`resolveFk`+FK, con 21 golden tests) era una **extracción** de la lógica REPLACE/preserve-on-missing que **YA vive inline** en `bulk-upload.js` (deployado, battle-tested). Nunca se cableó (era duplicado, dev-only, no referenciado en `config` → jamás se deployó). **Chequeo de drift (2026-07-23):** `decideLabelIds` (L5669-5681), `decideDims` (L5977-5989), `decideOptInOuts` (L6026-6042) = **idénticos** al inline; pero **`decideDimValueIds` DRIFTEÓ** (el inline evolucionó a `Parse.resolveDimSelections` con `deptoDefaultId`/`applyDeptoDefault` — cablear el módulo REGRESARÍA) y los FK de proceso/grupo quedaron como fragmentos de fallback. Como el comportamiento correcto ya está en producción inline y el módulo estaba parcialmente stale, **se borró el módulo + `tools/test_bulk_upload_build.js`** para eliminar el cabo suelto. Si a futuro se quiere deduplicar, es una tarea formal (re-sincronizar `decideDimValueIds`, completar FK, cablear, verificar equivalencia + corrida real).
