# Runbook — Sembrado de sentinelas (hash-autopilot v2, Fase C)

**Qué es:** un **sentinela** es un objeto canario en el ERP, marcado inequívocamente, que el autopilot puede **desarchivar → mutar → capturar el hash → re-archivar** para regenerar el hash de una mutation rotada, **sin tocar datos reales**. Este runbook explica cómo sembrarlos con seguridad.

> ⚠️ **Por qué importa la precisión:** el ciclo corre **desatendido en producción**. Si un sentinela se confunde con un objeto real, el autopilot mutaría datos productivos. Las salvaguardas de código (`isSentinel` fail-closed, blast-radius, journal) son la última línea; **el sembrado correcto es la primera**.
...
4. **En reposo = archivado.** El estado base del sentinela es `archived`. El ciclo lo desarchiva, muta y re-archiva; entre corridas siempre debe quedar archivado.
5. **Sin dependencias.** El sentinela no debe estar ligado a otras entidades (una OV sentinela sin partes reales, un PN sentinela sin inventario, etc.) para que archivar/mutar no arrastre efectos.
6. **Solo mutations reversibles.** Fase C v1 cubre `Save*/Update*/Archive*/Set*/Create*/Add*` (estrategia *archived-mutate-restore*). Las **destructivas** (`Delete*/Remove*`) **escalan** — NO se siembra sentinela para ellas (no hay forma segura de re-archivar algo borrado).
...
4. **Regístralo en `tools/hash-autopilot/sentinels-config.json`:**
   ```json
   {
     "entities": {
       "ReceivedOrder": {
         "id": "<idInDomain-o-nano-del-sentinela>",
...
> No hace falta sembrar las 69 de golpe. Empieza por 3-5 entidades de alto impacto; las demás **escalan** por correo hasta tener sentinela (comportamiento seguro por defecto).

## Verificación post-sembrado

- `sentinels-config.json` parsea y cada entrada tiene `id/marker/baseState/module`.
- Cada objeto sentinela existe, está **archivado**, y `isSentinel` da `true`.
- Corrida en seco del motor: con sentinela declarado, la mutation correspondiente pasa de `escalate` a `run` en el plan (sin ejecutar aún).

## Mantenimiento

- **Tras cada corrida real**, confirma que el sentinela quedó **archivado** (el ciclo lo re-archiva; la verificación post-ciclo alerta si no).
...
- ❌ No marques un objeto real como sentinela "temporalmente".
- ❌ No siembres sentinela para `Delete*/Remove*` (escalan por diseño).
- ❌ No dejes el sentinela desarchivado ni con relaciones a datos reales.
- ❌ No reutilices un sentinela entre tipos de entidad distintos.
